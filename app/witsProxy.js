  ...
  const connectToTcpServer = () => {
    ...
    if (config.enableMultiplexing) {
      ...
      if (sharedConnection && sharedConnection.socket.writable) {
        ...
        // Implement data recovery mechanism - send recent data from circular buffer
        const tcpKey = `${tcpHost}:${tcpPort}`;
        if (dataBuffers.has(tcpKey)) {
          const recentData = dataBuffers.get(tcpKey).getAll();
          if (recentData.length > 0) {
            // Send notification about available historical data
            ws.send(
              JSON.stringify({
                type: "recovery",
                status: "available",
                recordCount: recentData.length,
                oldestTimestamp: recentData[0].timestamp,
                newestTimestamp: recentData[recentData.length - 1].timestamp,
              }),
            );
          }
        }
